...
public class AdminPerformService {
	...
	public int searchPerfTotalCnt(AdminPerfRangeDTO prDTO) {
		int cnt=0;
		AdminPerformDAO pDAO = AdminPerformDAO.getInstance();
		
		try {
			cnt = pDAO.selectPerfTotalCnt(prDTO);
	...
	public List<AdminPerformDTO> searchPerform(AdminPerfRangeDTO prDTO){
		List<AdminPerformDTO> list = new ArrayList<AdminPerformDTO>();
		
		AdminPerformDAO pDAO = AdminPerformDAO.getInstance();
		
		try {
			list = pDAO.selectPerform(prDTO);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public AdminPerformDTO searchDetailPerform(int num){
		AdminPerformDTO pDTO = new AdminPerformDTO();
		AdminPerformDAO pDAO = AdminPerformDAO.getInstance();
		
		try {
			pDTO = pDAO.selectDetailPerform(num);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public int modifyPerform(AdminPerformDTO pDTO) {
	...
	public int removePerform(int perfNum) {
		int flag=0;
		
		AdminPerformDAO pDAO = AdminPerformDAO.getInstance();
		
		try {
			flag = pDAO.updateDelPerform(perfNum);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public int addPerform(AdminPerformDTO pDTO) {
		int flag = 0;
		
		AdminPerformDAO pDAO = AdminPerformDAO.getInstance();
		try {
			pDAO.insertPerform(pDTO);
			flag=3;
		} catch (SQLException e) {
			e.printStackTrace();
		}//end catch
	...
	public String pagination2( AdminPerfRangeDTO rDTO ) {
