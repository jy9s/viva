...
private static AdminFaqService ns;
	
	private AdminFaqService() {
		
	}
	
	
	public static AdminFaqService getInstance() {
		if(ns==null) {
			ns=new AdminFaqService();
		}
	...
	public void titleSubStr(List<AdminFaqDTO> faqList) {
		String title="";
		for(AdminFaqDTO fDTO : faqList){
		title = fDTO.getFaqTitle();
			if(title.length()>39){
				fDTO.setFaqTitle(title.substring(0,40)+"...");
			}
		}
	}//end if }//end for }
	...
	public int totalCnt(AdminFaqRangeDTO rDTO) {
		int totalCnt=0;
		
		AdminFaqDAO nDAO = AdminFaqDAO.getInstance();
		
		
		try {
			totalCnt = nDAO.selectFaqTotalCnt(rDTO);
	...
	public boolean addFaq(AdminFaqDTO nDTO) {
		boolean flag = false;
	 ...
	 * @return
	 */
	public List<AdminFaqDTO> searchFaqList(AdminFaqRangeDTO rDTO){
		List<AdminFaqDTO> list = null;
		AdminFaqDAO nDAO = AdminFaqDAO.getInstance();
		try {
	 ...
	 * @return
	 */
	public AdminFaqDTO searchOneFaq(int num) {
	 ...
	 * @return
	 */
	public String pagination( AdminFaqRangeDTO rDTO, String justify ) {
	...
	public boolean removeFaq(int num) {
		boolean flag=false;
	...
	public boolean modifyFaq(AdminFaqDTO nDTO) {
		boolean flag=false;
		
		AdminFaqDAO nDAO = AdminFaqDAO.getInstance();
		try {
			flag = nDAO.updateFaq(nDTO)==1;
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return flag;
