	...
	$(function() {
     ...
     // 검색창 선택시 카테고리 적용 되게 하기		
		...
		$("tbody").on("click", "tr > .noref", function () {
			...
			window.open("detail_receipt.jsp?num="+num+"&type=perf&name="+name ,"idWin","width=500px, height=800px,top="+(window.screenY+120)+", left="+(window.screenX+260));
...
<body id="page-top">
	...
	<div id="wrapper">
		...
		int totalCount = arService.totalCnt(rDTO,"perf_reserve");
		...
		List<AdminReserveDTO> perfList = arService.searchReserveList(rDTO,"perf_reserve");
		
		//arService.titleSubStr(perfList);
		rDTO.setUrl("perf_reserve.jsp");
		...
		pageContext.setAttribute("perfList", perfList);
		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<h1 class="h3 mb-0 text-gray-800 m-0 font-weight-bold text-danger">공연 예매 관리</h1>
					<form id="searchFrm" action="perf_reserve.jsp">
					<div
						...
						<div class="dropdown search-filter">
							<button class="btn btn-danger dropdown-toggle" type="button"
								data-toggle="dropdown" aria-expanded="false" id="field_text">예약번호</button>
								<input type="hidden" name="field" id="field_value" value="1">
							<ul class="dropdown-menu">
								<li><a class="dropdown-item" data-value="1">예약번호</a></li>
						...
						<div class="card-body">
							<div class="table-responsive">
								<table class="table table-bordered table-hover" id="dataTable" width="100%"
									...
									<tbody>
										<c:if test="${ empty perfList }">
										<tr>
										<td colspan="10" style="text-align:center">작성된 게시글이 없습니다.</td>
										<tr>
										</c:if>
										<c:forEach var="perfDTO" items="${perfList}" varStatus="i">
										<tr>
										
										<td style="text-align:center; height:60px">
										<div class="reserve-num" data-reserve-num="${perfDTO.resNum}" data-reserve-name="${perfDTO.resName}"><c:out value="${perfDTO.resNum}"/></div>										
										</td>
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resName}"/></td>
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resId}"/></td>
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resCount}"/></td>
										<td style="text-align:center" class="noref"><fmt:formatNumber value="${perfDTO.resPrice}" pattern="#,###"/></td>								
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resTel}"/></td>
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resPayMethod}"/></td>
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resPayDate}"/></td>
										<td style="text-align:center" class="noref"><c:out value="${perfDTO.resRefund}"/></td>										
										
										<c:choose>
										<c:when test="${perfDTO.resRefund eq 'N' }">																			
										<td style="text-align:center">
										<input type="button" name="${perfDTO.resNum}" class="btn btn-danger refund-btn" value="환불">
										...
										<c:otherwise>
										<td style="text-align:center">환불됨</td>
										</c:otherwise>
										</c:choose>
										</tr>										
										</c:forEach>
