...
if(serviceFlag.equals("3")){ //delete
	deleteFlag = fService.removeFaq(tempNum);	
}else{

nDTO.setFaqNum(tempNum);
nDTO.setFaqTitle(title);
nDTO.setFaqMsg(intro);
nDTO.setFaqType(category);
}
...
<c:choose>
	...
	alert("내용을 삭제하였습니다");
	location.href="faq.jsp?currentPage=${currentPage}&field=${field}&keyword=${keyword}";
	</c:when>
	<c:when test="${empty nDTO.faqMsg}">
	alert("내용을 입력해주세요.");
	history.back();
	</c:when>
	<c:when test="${nDTO.faqMsg eq '<br>'}">
	alert("내용을 입력해주세요.");
	history.back();
	</c:when>
	<c:when test="${nDTO.faqMsg eq '<p><br></p>'}">
	alert("내용을 입력해주세요.");
	history.back();
	</c:when>
	<c:when test="${empty nDTO.faqTitle}">
	alert("제목을 입력해주세요.");
	history.back();
	</c:when>
<c:otherwise>
	...
	if(serviceFlag.equals("1")){
		addFlag = fService.addFaq(nDTO);
	}
	
	if(serviceFlag.equals("2")){
		modifyFlag = fService.modifyFaq(nDTO);
	}
